...
from tools.script_parser import extract_scenes_gemini, extract_text_from_pdf
...
async def run_pipeline(
    ...
    scenes = await extract_scenes_gemini(raw_text, api_key)
    if not scenes:
        raise ValueError("No scenes could be extracted from the screenplay.")

    region_lat, region_lng = await geocode_region(base_region, api_key)
    ...
    shoot_order = await calculate_pairwise_travel(shoot_order, api_key)
